...
class Player(object):
    ...
    def go_to(self, direction):
        ...
        if direction.name == self.place.name:
            print(f"Can't go to {direction.name} from {direction.name}.")
            
            return
        else:
            print(f"You are at {direction.name}.")
    ...
    def talk_to(self, person):
        ...
        for m in mates:
            if person == m.name:
                ...
                exit = True
        if not exit:
            print("Not a valid mate!")
            return
        for c in self.place.characters:
            if person.name == c:
                print(f"{person.name} says: {person.talk()}")
                return
        print(f"{person.name} is not here.")
        return
    ...
    def take(self, thing):
        ...
        for p in self.place.things:
            if thing.name == p:
                print(f"{self.name} takes the {thing.name}.")
                self.backpack.append(thing)
                self.place.things.pop(thing.name)
                return
        
        print(f"{thing.name} is not here.")
        return
...
mates = [randy, albert, yulin, jeffrey, derrick, ken, student]
...
Places = [tuc, ccm, erc, daap, bcc, baldwin, game_store, starbucks, swift, rhodes, steger, nippert]
